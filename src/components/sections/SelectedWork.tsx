...
import { ArrowUpRight, Sparkles, Compass, FolderLock } from 'lucide-react';
...
export const SelectedWork: React.FC<SelectedWorkProps> = ({
  ...
  return (
    <section id="work" className="relative w-full py-24 sm:py-36 px-4 sm:px-8 bg-[#FAF9F6] border-b border-avora-border">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 pb-8 border-b border-avora-border">
          ...
          <div className="max-w-md space-y-4">
            <p className="text-sm sm:text-base font-sans text-avora-muted leading-relaxed">
              An archival collection of brand identities, digital experiences, streetwear capsules, and creative systems.
            </p>

            {/* Filter Pills (active when projects exist) */}
            {hasRealProjects && (
        ...
        {/* Dynamic State: Intentional Curated Empty State vs Real Projects Grid */}
        {!hasRealProjects ? (
          /* Intentional Luxury Curated State (Honest & Uncompromising) */
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="relative w-full py-24 sm:py-32 px-6 sm:px-12 rounded-3xl bg-[#F6F4EE]/60 border border-avora-border flex flex-col items-center justify-center text-center space-y-6 shadow-xs overflow-hidden"
          >
            ...
            <h3 className="font-serif text-3xl sm:text-5xl md:text-6xl font-bold tracking-tight text-avora-charcoal max-w-2xl leading-tight">
              THE ARCHIVE IS BEING CURATED.
            </h3>

            <p className="text-sm sm:text-base font-sans text-avora-muted max-w-xl leading-relaxed">
              Real client commissions, case studies, and multidisciplinary identity systems will appear here soon.
            </p>

            <div className="pt-4 flex flex-wrap items-center justify-center gap-4">
              <button
                onClick={() => onNavigate?.('consultation')}
                className="px-7 py-3.5 rounded-full bg-avora-charcoal text-white text-xs font-sans font-semibold tracking-wide hover:bg-black transition-all flex items-center gap-2 shadow"
              >
              ...
              <button
                ...
                className="px-7 py-3.5 rounded-full bg-white border border-avora-border text-avora-charcoal text-xs font-sans font-medium tracking-wide hover:bg-avora-ivory transition-all flex items-center gap-2 shadow-xs"
              >
                <span>Start a Project</span>
                <ArrowUpRight className="w-3.5 h-3.5 text-avora-muted" />
              </button>
            </div>

            {/* Disciplines Ready */}
            <div className="pt-8 border-t border-avora-border-light text-[11px] font-mono text-avora-muted flex flex-wrap items-center justify-center gap-4">
              <span>BRANDING</span>
              <span>•</span>
          ...
          /* Asymmetric Editorial Portfolio Grid (When real projects are added) */
      ...
      {/* Case Study Modal Presentation (Active when project is clicked) */}
      <CaseStudyModal
